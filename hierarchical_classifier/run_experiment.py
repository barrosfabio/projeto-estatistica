...
resampling_algorithms = [RANDOM_OVERSAMPLER]
...
for i in range(0,2):
    ...
    for strategy in resampling_strategies:
        ...
        if strategy != NONE:
            for algorithm in resampling_algorithms:
                ...
                [hierarchical_result, flat_result, result_per_class] = experimental_protocol.run_cross_validation(folds, algorithm, classifier_name, strategy)

                hierarchical_results_list.append(hierarchical_result)
                flat_results_list.append(flat_result)
                results_per_class_list.append(result_per_class)
        else:
            experimental_protocol = ExperimentalProtocol(input_data, output_data, unique_classes, classifier_name)
            [hierarchical_result, flat_result, result_per_class] = experimental_protocol.run_cross_validation(folds, NONE, classifier_name,
                                                                                                 strategy)
    ...
    result_framework.transform_to_csv(hierarchical_results_list, 'experiment_results_hierarchical')
    result_framework.transform_to_csv(flat_results_list, 'experiment_results_flat')
